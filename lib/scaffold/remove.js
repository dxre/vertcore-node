...
function remove(options, done) {
  ...
  var vertcoreConfigPath = path.resolve(configPath, 'vertcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(vertcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to vertcore-node.json
        removeConfig(vertcoreConfigPath, service, next);
      });
    }, done
  );
}
